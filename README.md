...
This is a Fedora release 23 VM that has walcyrie already compiled and ready to use.  You will need to install the Perl module Text::CSV (cpan> install Text::CSV). To run the experiments, log in as ppopp16 (passwd: ae), and simply run ~/walcyrie/bin/experiments.sh. The $ARTEFACT_ROOT is set to ~/walcyrie.
